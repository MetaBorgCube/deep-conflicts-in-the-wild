...
public class SGLR {
    ...
    Set<Integer> productionsUsedEachParse = Sets.newHashSet();
    Set<Integer> productionsUsedAllParses = Sets.newHashSet();
    ...
    private void printProductionsUsed() {
        if(parseTable.getPTgenerator() != null) {
            for(Integer prod : productionsUsedEachParse) {
                System.out.println(parseTable.getPTgenerator().normalizedGrammar().getProdLabels().inverse().get(prod));
            }
        }
    ...
    private void initParseVariables(String input, String filename) {
        productionsUsedEachParse.clear();
        forActor.clear();
                ...
                for(final ActionItem ai : action.getActionItems()) {
                    switch(ai.type) {
                        ...
                        case ActionItem.REDUCE: {
                            ...
                            productionsUsedEachParse.add(red.label);
                            productionsUsedAllParses.add(red.label);
    ...
    public int numberOfProductionsUsedSingleParse() {
        return productionsUsedEachParse.size();
    }
    
    public int numberOfProductionsUsedAllParses() {
        return productionsUsedAllParses.size();
    }
